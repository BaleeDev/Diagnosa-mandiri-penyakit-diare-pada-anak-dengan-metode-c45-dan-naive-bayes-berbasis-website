...
if (@$_SESSION['Admin'] || @$_SESSION['Pegawai']) {
    ...
    function tampilDataAtribut($NamaAtribut1, $NamaAtribut2)
    {
        ...
        if ($NamaAtribut1 == "") {
            ...
            $tampil = mysqli_query($koneksi, "SELECT*FROM tbl_atribut WHERE Nama_atribut != '$NamaAtribut1' && Nama_atribut != '$NamaAtribut2'");
        } else {
            $tampil = mysqli_query($koneksi, "SELECT*FROM tbl_atribut WHERE Nama_atribut != '$NamaAtribut1'");
        }
        return $tampil;
    }
                ...
                <tbody>
                    ...
                    if ($tampil) {
                        if (mysqli_num_rows($tampil) > 0) {
                            // lakukan perulangan selama data pada database tbl_data ada
                            while ($row = mysqli_fetch_array($tampil)) {
                                $vHasil = cekRules($row['Derajat_Haus'], $row['Kualitas_Denyut_Nadi'], $row['Palpebra'], $row['Frekuensi_Denyut_Jantung'], $row['Status_Mental']);
                    ?>
                                <tr>
                                    <th scope="row"><?= $nomor++ ?></th>
                                    <td><?php echo $row['Jenis_Kelamin'] ?></td>
